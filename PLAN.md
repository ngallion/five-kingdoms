...
- [ ] **Faction files** (`world/factions/`): the king's movement (it needs a name — his followers don't call themselves evil), the Agoran resistance/exiles, the Forgeholm resistance, and at least one cross-kingdom faction that predates the isolation (old traders' guild? a courier order that still crosses borders?) — the party's natural infrastructure.
